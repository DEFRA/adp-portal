...
const isInPlatformAdminGroupSpy = jest.spyOn(
  RbacUtilities.prototype,
  'isInPlatformAdminGroup',
);
const isIsInProgrammeAdminGroupSpy = jest.spyOn(
  RbacUtilities.prototype,
  'isInProgrammeAdminGroup',
);
const isIsInAdpUserGroupSpy = jest.spyOn(
  RbacUtilities.prototype,
  'isInAdpUserGroup',
);
...
describe('adpPortalPermissionPolicy: Platform Admin User', () => {
  ...
  it.each([
    ...
    async ({ permission, expected }) => {
      ...
      const mockRbacUtilities = new RbacUtilities(mockLogger, mockRbacGroups);
      const policy = new AdpPortalPermissionPolicy(
        mockRbacUtilities,
        mockLogger,
      );
      const request: PolicyQuery = { permission: permission };

      const policyResult = await policy.handle(
        request,
        mockPlatformAdminUserResponse,
      );

      expect(isInPlatformAdminGroupSpy).toHaveBeenCalled();
      expect(isInPlatformAdminGroupSpy).toHaveBeenCalledWith(
        mockPlatformAdminUserResponse,
      );
